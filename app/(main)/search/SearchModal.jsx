              ...
              {!loading && results.length > 0 && (
                <ul className="space-y-3">
                  {results.slice(0, 6).map(product => (
                    <li key={product.id}>
                      <Link href={`/search?query=${encodeURIComponent(product.name)}`} onClick={onClose} className="flex items-center gap-4 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
